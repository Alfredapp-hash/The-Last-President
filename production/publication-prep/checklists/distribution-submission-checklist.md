# Distribution Submission Checklist

Use after EPUB and print files are validated.

## 1) Metadata lock

- [x] `metadata/retailer-metadata.csv` completed (no `TBD` values)
- [x] ISBNs assigned per format
- [ ] BISAC categories verified against target retailer taxonomy
- [ ] Keywords reviewed for platform-specific limits
- [ ] Author name, series name, subtitles, and casing normalized

## 2) Asset lock

- [ ] Final cover JPG/PNG uploaded per book
- [ ] Final titled cover variants match metadata title/subtitle exactly
- [ ] OG/social assets published in press kit folder
- [ ] Author photo and bio finalized (if required by retailer)

## 3) File package lock

- [x] EPUB file paths:
  - [x] Book One: `production/publication-prep/exports/epub/baren-sump-book-1-the-last-president_v1.epub`
  - [x] Book Two: `production/publication-prep/exports/epub/baren-sump-book-2-children-of-tomorrow_v1.epub`
  - [x] Book Three: `production/publication-prep/exports/epub/baren-sump-book-3-the-black-path_v1.epub`
- [x] Print interior PDF paths:
  - [x] Book One: `production/publication-prep/exports/pdf/baren-sump-book-1-interior-print_v1.pdf`
  - [x] Book Two: `production/publication-prep/exports/pdf/baren-sump-book-2-interior-print_v1.pdf`
  - [x] Book Three: `production/publication-prep/exports/pdf/baren-sump-book-3-interior-print_v1.pdf`
- [x] Print cover wrap PDF paths:
  - [x] Book One: `production/publication-prep/exports/covers/the-last-president-cover-wrap_v1.pdf`
  - [x] Book Two: `production/publication-prep/exports/covers/children-of-tomorrow-cover-wrap_v1.pdf`
  - [x] Book Three: `production/publication-prep/exports/covers/the-black-path-cover-wrap_v1.pdf`

## 4) Platform submissions

- [ ] Amazon KDP
- [ ] Apple Books
- [ ] Kobo Writing Life
- [ ] Google Play Books
- [ ] IngramSpark (if print + expanded distribution)

## 5) Store-page QA

- [ ] Series order visible and correct
- [ ] Descriptions rendered without broken formatting
- [ ] Preview pages display first chapter correctly
- [ ] Categories and keywords attached correctly
- [ ] Author page links resolve

## 6) Final go/no-go sign-off

- [ ] No unresolved platform warnings
- [x] No file validation errors
- [ ] Launch date confirmed
- [ ] Team/author sign-off captured

## Notes

- Keep this checklist under version control and update after each platform upload.
